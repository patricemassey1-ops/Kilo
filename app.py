...
if "historique" not in st.session_state:
    st.session_state["historique"] = charger_donnees(
        DB_FILE, ["Joueur", "Salaire", "Statut", "Pos", "Propriétaire", "pos_order"]
    )
...
fichiers_telecharges = st.sidebar.file_uploader(
    "📥 Importer CSV Fantrax", type="csv", accept_multiple_files=True
)
...
if fichiers_telecharges:
    ...
    for idx, fichier in enumerate(fichiers_telecharges):
        try:
            ...
            def extract_table(lines, keyword):
                ...
                h_idx = next(
                    (
                        ...
                        if any(kw in lines[i] for kw in ["ID", "Player", "Salary"])
                    ),
                    -1,
                )
                if h_idx == -1:
            ...
            df_merged = pd.concat(
                [extract_table(lines, "Skaters"), extract_table(lines, "Goalies")],
                ignore_index=True,
            )
                ...
                temp_df = pd.DataFrame(
                    {
                        ...
                        "Statut": df_merged[c_status].apply(
    ...
    if dfs_a_ajouter:
        ...
        st.session_state["historique"] = (
            pd.concat([st.session_state["historique"], new_data], ignore_index=True)
...
with tab1:
    if not st.session_state["historique"].empty:
        ...
        summary = (
            temp_df.groupby(["Propriétaire", "Propriétaire_nom", "DateTime", "Statut"], observed=True)[
                "Salaire"
            ]
            .sum()
            .reset_index()
                ...
                for i, col_name in enumerate(row.index):
                    if col_name in ["Grand Club", "Restant Grand Club"]:
                        styles[i] = (
                            ...
                            if gc_depasse
                            else "color: #00cc00; font-weight: bold"
                        )
        ...
        proprietaires_display = [
            f"{row['Propriétaire_nom']} ({row['DateTime']})" for _, row in summary.iterrows()
        ]
        ...
        for _, row in summary.iterrows():
            ...
            if row["Restant Grand Club"] < 0:
                alertes.append(
                    f"🚨 **{proprio_display}** dépasse le plafond du Grand Club de **{format_currency(abs(row['Restant Grand Club']))}**"
                )
            if row["Restant Club École"] < 0:
                alertes.append(
                    f"🚨 **{proprio_display}** dépasse le plafond du Club École de **{format_currency(abs(row['Restant Club École']))}**"
                )
        ...
        if not proprietaires_uniques:
            ...
            if selected_proprio_full:
                ...
                if f"{sim_key}_grand_club" not in st.session_state:
                    ...
                    st.session_state[f"{sim_key}_grand_club"] = [
                        j for j in gc_joueurs if j and str(j).strip() != "" and str(j) != "0"
                    ]
                if f"{sim_key}_club_ecole" not in st.session_state:
                    ...
                    st.session_state[f"{sim_key}_club_ecole"] = [
                        j for j in ce_joueurs if j and str(j).strip() != "" and str(j) != "0"
                    ]
                    ...
                    for joueur_nom in st.session_state[f"{sim_key}_grand_club"]:
                        ...
                        if not joueur_info.empty:
                            ...
                            joueurs_gc_data.append(
                                {"Joueur": joueur_nom, "Pos": j["Pos"], "Salaire": format_currency(salaire)}
                            )
